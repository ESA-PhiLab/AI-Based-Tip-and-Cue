...
def get_landsat_satellites() -> list[SatelliteInfo]:
    """Return Landsat 7, 8, and 9 with valid time ranges."""
    return [
        SatelliteInfo(
            name="Landsat-7",
            norad_id=25682,
            earliest_valid_utc=datetime(1999, 4, 15, 0, 0, 0, tzinfo=timezone.utc),
            decay_date_utc=datetime(2025, 6, 4, 13, 8, 0, tzinfo=timezone.utc),
        ),
        SatelliteInfo(
            name="Landsat-8",
            norad_id=39084,
            earliest_valid_utc=datetime(2013, 2, 11, 0, 0, 0, tzinfo=timezone.utc),
        ),
        SatelliteInfo(
            name="Landsat-9",
            norad_id=49260,
            earliest_valid_utc=datetime(2021, 9, 27, 0, 0, 0, tzinfo=timezone.utc),
        ),
    ]
...
if __name__ == "__main__":
    ...
    satellites = get_landsat_satellites()
