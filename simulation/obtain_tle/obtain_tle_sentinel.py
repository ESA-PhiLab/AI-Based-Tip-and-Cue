...
def get_sentinel2_satellites() -> list[SatelliteInfo]:
    """Return Sentinel-2A, 2B, and 2C with valid time ranges."""
    return [
        SatelliteInfo(
            name="Sentinel-2A",
            norad_id=40697,
            earliest_valid_utc=datetime(2015, 6, 23, 0, 0, 0, tzinfo=timezone.utc),
        ),
        SatelliteInfo(
            name="Sentinel-2B",
            norad_id=42063,
            earliest_valid_utc=datetime(2017, 3, 7, 0, 0, 0, tzinfo=timezone.utc),
        ),
        SatelliteInfo(
            name="Sentinel-2C",
            norad_id=60989,
            earliest_valid_utc=datetime(2024, 9, 5, 0, 0, 0, tzinfo=timezone.utc),
        ),
    ]
...
if __name__ == "__main__":
    ...
    satellites = get_sentinel2_satellites()
