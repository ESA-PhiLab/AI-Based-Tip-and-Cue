...
def points_array_from_targets(targets, t_datetime):
    """
    Returns an (N,3) array of ECI points for:
      - dict of targets (possibly empty)
      - single target dict with 'lat','lon','alt'
    """
    if isinstance(targets, dict):
        if len(targets) == 0:
            return np.empty((0, 3))
        return whales_to_points_eci(targets, t_datetime)
    # single target dict
    pts = whales_to_points_eci(targets, t_datetime)
    pts = np.asarray(pts)
    if pts.ndim == 1:
        pts = pts.reshape(1, 3)
    return pts
