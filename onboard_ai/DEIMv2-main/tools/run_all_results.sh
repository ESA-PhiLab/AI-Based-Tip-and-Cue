...
# Runs main.sh once per existing subfolder in <repo_root>/results.
# Uses the folder name as RUN_NAME so each run writes into that folder.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

MAIN="${REPO_ROOT}/tools/main_local.sh"   # <-- adjust if your main.sh lives elsewhere
RESULTS_DIR="${REPO_ROOT}/results"

if [[ ! -x "${MAIN}" ]]; then
  echo "[run_all_results.sh] ERROR: main.sh not found/executable at: ${MAIN}"
