...
# =========================
def display_images_in_grid(images: list):
...
# Main
# =========================
def show_data(n: int = 1, segmentation: bool = False) -> None:
    """show_data() -> None: Load images and show grid."""
    ...
    for i in range(n):
        if i < len(subdirs):  # Make sure we don't exceed the available subdirectories
            ...
            print(f"Displaying plot {i + 1}")
            display_images_in_grid(loaded_images)
        else:
...
if __name__ == "__main__":
    ...
    show_data(n=5, segmentation = True)