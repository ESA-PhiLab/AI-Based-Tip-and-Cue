...
# =========================
# If cuda_ad_rgb still crashes on your system, switch to "cuda_rgb" (often more stable).
...
# COCO helpers
# =========================
def load_json(path: Path) -> dict:  # noqa: D401
...
# =========================
def main() -> None:
    """main() -> None: Translate per-instance annotations to off-nadir resolution, then run generate_image and overlay."""
    ...
    # ==========================================================
    print("[4/7] Translating segmentation per annotation (no global mask)...")
    ...
    total_polys = sum(len(v) for v in polys_off_by_ann.values())
    print(f"      Per-instance polygons extracted: {total_polys} (from {len(polys_off_by_ann)} anns)")

    # ==========================================================
    # (B) BBOX: ID corners at off-nadir res (multi-instance OK)
    # ==========================================================
    ...
    print(f"[6/7] Off-nadir boxes extracted: {len(boxes_off)}")
    ...
    # ==========================================================
    coco_out = dict(coco)  # shallow copy is fine for printing

    # carry over any existing top-level keys (images/annotations/categories/licenses/info, etc.)
    images_out = list(coco.get("images", []))
    anns_out = list(coco.get("annotations", []))

    new_image_id = int(max([x.get("id", 0) for x in images_out] + [0]) + 1)

    # You can set file_name to whatever you will save the off-nadir render as.
    # Keeping it deterministic helps.
    offnadir_file_name = str(Path("offnadir_renders") / Path(IMG_FILE).with_suffix(".png").name)

    images_out.append({
        "id": new_image_id,
        "license": im.get("license", 1),
        "file_name": offnadir_file_name,
        "height": int(RENDER_RESOLUTION),
        "width": int(RENDER_RESOLUTION),
        "date_captured": DATETIME_UTC.isoformat().replace("+00:00", "Z"),
        "extra": {
            "source_file_name": IMG_FILE,
            "render_resolution": int(RENDER_RESOLUTION),
            "variant": MI_VARIANT,
        },
    })

    next_ann_id = int(max([a.get("id", 0) for a in anns_out] + [0]) + 1)

    # Build translated annotations (per instance)
    translated_anns = []
    for ann in anns:
        ...
        bbox = boxes_off_by_ann.get(aid, (0.0, 0.0, 0.0, 0.0))

        translated_anns.append({
            "id": next_ann_id,
            "image_id": new_image_id,
            "category_id": int(ann.get("category_id", 1)),
            "segmentation": polygons_to_coco_segmentation(polys),
            "area": float(ann.get("area", 0.0)),  # optional; keep if you want, or recompute later
            "bbox": [float(bbox[0]), float(bbox[1]), float(bbox[2]), float(bbox[3])],
            "iscrowd": int(ann.get("iscrowd", 0)),
        })
        next_ann_id += 1

    anns_out.extend(translated_anns)

    coco_out["images"] = images_out
    coco_out["annotations"] = anns_out

    print(json.dumps(coco_out, indent=2))
