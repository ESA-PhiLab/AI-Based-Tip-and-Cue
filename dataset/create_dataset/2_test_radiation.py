...
dt = (
        datetime.fromisoformat(datetime_utc.replace("Z", "+00:00"))
        if datetime_utc
        else datetime(2025, 6, 11, 8, 0, 0, tzinfo=timezone.utc)
    )

DN255_rgb_offnadir, DN255_rgb_sunglint, radiance_sunglint, DN255_combined = generate_image(img_path, satellite, sat_lat, sat_lon, sat_alt, tgt_lat, tgt_lon, tgt_alt, dt, sensor_characteristics, wave_properties, bools, seed_dem)
